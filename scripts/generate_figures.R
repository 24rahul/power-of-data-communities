...
# ===========================================================================
# Export figure data for manuscript (Results section)
# ===========================================================================
figure_data_dir <- file.path(base_dir, "manuscript", "figure_data")
dir.create(figure_data_dir, showWarnings = FALSE, recursive = TRUE)

# Figure 1: Cumulative first-degree (citing) papers over time (monthly)
fig1m_export <- fig1m_data
fig1m_export$dataset <- as.character(fig1m_export$dataset)
write.csv(fig1m_export, file.path(figure_data_dir, "figure1_cumulative_citing_papers_monthly.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/figure1_cumulative_citing_papers_monthly.csv\n")

# Figure 2: First- and second-degree citation counts (absolute and per $1M funding)
ripple_export <- ripple_summary
ripple_export$dataset <- as.character(ripple_export$dataset)
write.csv(ripple_export, file.path(figure_data_dir, "figure2_ripple_summary.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/figure2_ripple_summary.csv\n")

# Figure 3: Cumulative funding-normalized citations over time (1st- and 2nd-degree per $1M)
ripple_time_export <- ripple_time
ripple_time_export$dataset <- as.character(ripple_time_export$dataset)
ripple_time_export$layer <- as.character(ripple_time_export$layer)
write.csv(ripple_time_export, file.path(figure_data_dir, "figure3_iceberg_cumulative_per_million_over_time.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/figure3_iceberg_cumulative_per_million_over_time.csv\n")
...
# Export supplementary figure data (S1, S2, S3)
# S1: Disciplinary diffusion (share of papers by primary field, by dataset and year)
field_year_export <- field_year
field_year_export$dataset <- as.character(field_year_export$dataset)
field_year_export$field_group <- as.character(field_year_export$field_group)
write.csv(field_year_export, file.path(figure_data_dir, "supplementary_figureS1_disciplinary_diffusion.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/supplementary_figureS1_disciplinary_diffusion.csv\n")

# S2: First-degree citation impact by rank (Panel A: citations, Panel B: citations per $1M)
fig2_export <- fig2_data
fig2_export$dataset <- as.character(fig2_export$dataset)
write.csv(fig2_export, file.path(figure_data_dir, "supplementary_figureS2_first_degree_citation_by_rank.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/supplementary_figureS2_first_degree_citation_by_rank.csv\n")

# S3: Second-degree paper distribution (Panel A: raw citations, Panel B: per $1M funding)
fig5_raw_export <- fig5_raw_data
fig5_raw_export$dataset <- as.character(fig5_raw_export$dataset)
write.csv(fig5_raw_export, file.path(figure_data_dir, "supplementary_figureS3_second_degree_distribution_raw.csv"), row.names = FALSE)
fig5_norm_export <- fig5_norm_data
fig5_norm_export$dataset <- as.character(fig5_norm_export$dataset)
write.csv(fig5_norm_export, file.path(figure_data_dir, "supplementary_figureS3_second_degree_distribution_per_million.csv"), row.names = FALSE)
cat("  Exported: manuscript/figure_data/supplementary_figureS3_second_degree_distribution_raw.csv & _per_million.csv\n")
